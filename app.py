...
<style>
    ...
    * { font-family: 'Outfit', sans-serif; box-sizing: border-box; }
    ...
    /* ══════════════════════════════════════════════════════════════════
       Gold-theme HTML tables (same as India Sales Report)
       ══════════════════════════════════════════════════════════════════ */
    .table-scroll {
        overflow-y: auto;
  ...
  <div class='kpi-value'>{value}</div>
            ...
            def grouped_bar_chart(data, x_col, title, xaxis_label):
                ...
                fig.add_trace(go.Bar(
                    name='Initial Qty',
                    x=data[x_col], y=data['INITIAL_QTY'],
                    text=data['INITIAL_QTY'].apply(lambda v: f"{v:,.0f}"),
                    marker=dict(color="#D4AF37", cornerradius=5,
                                line=dict(color='rgba(255,255,255,0.06)', width=1)),
                ))
                fig.add_trace(go.Bar(
                    name='Balance Qty',
                    x=data[x_col], y=data['BALANCE'],
                    text=data['BALANCE'].apply(lambda v: f"{v:,.0f}"),
                    marker=dict(color="#E67E22", cornerradius=5,
                                line=dict(color='rgba(255,255,255,0.06)', width=1)),
                ))
                fig.add_trace(go.Bar(
                    name='Damaged Qty',
                    x=data[x_col], y=data['DAMAGED_QTY'],
                    text=data['DAMAGED_QTY'].apply(lambda v: f"{v:,.0f}"),
                    marker=dict(color="#C0392B", cornerradius=5,
                                line=dict(color='rgba(255,255,255,0.06)', width=1)),
                ))
                fig.update_layout(barmode='group', title=title)
                fig = _dark_layout(
                    fig, xaxis_label, "Quantity",
                    extra_xaxis={'categoryorder': 'array',
                                 'categoryarray': data[x_col].tolist()},
                    height=520,
                    show_legend=True,
                )
...
`COLAB` · `COLOR` · `BRAND` · `SEASON` · `Category` · `SUB CATEGORY` · `INITIAL QTY` · `Total Qty` · `Balance` · `Damaged Qty`
